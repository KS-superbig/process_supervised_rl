...
得到的经验是：轻量 LoRA 很适合 SFT warmup，因为 SFT 主要做格式、风格和领域冷启动；但 GRPO 需要在采样噪声和 reward 信号下真正改变策略。早期 small-rank、约 200-step 的 RL 只能算 smoke test，容量和训练强度都不足。因此先将 SFT adapter merge 到基座，再挂 fresh `r=512`、`alpha=1024` 的 RL LoRA，并将训练拉到 1000 steps；步数太少也会削弱 RL 更新的效果。
